...
def capture_screenshot(page, url: str, destination: Path, timeout_ms: int) -> str | None:
    try:
    ...
    with sync_playwright() as p:
        ...
        for source in sources:
            ...
            for entry in entries:
                ...
                if link and has_screenshot_budget:
                    filename = f"{screenshot_count + 1:03d}-{slugify(title)}.png"
                    filepath = screenshots_dir / filename
                    screenshot_name = capture_screenshot(page, link, filepath, timeout_ms)
                    if screenshot_name:
                        item["screenshot"] = f"screenshots/{screenshot_name}"
                        screenshot_count += 1
                elif link and screenshot_limit is not None and not screenshot_limit_reached_logged:
                    LOGGER.info(
